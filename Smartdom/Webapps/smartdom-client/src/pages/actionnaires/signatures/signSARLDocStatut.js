...
class signSARLDocStatut extends Component {
    ...
    componentWillMount() {
        ...
        firebase.database().ref('/society/' + SocietyUID).on('value', (snapshot) => {
           ...
           // this.setActio(data, actID);
            ...
            /*if (data.signatures !== null && data.signatures !== undefined) {

                //vérifier si tous les actionnaires ont signeé le doc statut
                if ((data.signatures.length === this.state.allActionnaire.length)) {
                    this.setState({
    ...
    componentDidMount() {
        ...
        augmCapitalService.getEntrepriseStatut(SocietyUID, pays).then(res => {
            this.setState({
                pdfFile: res,
                uid: SocietyUID,
                pays: pays,
    ...
    handlePDFChange = (name, eseName, url) => {
        ...
        let gerant = this.state.gerants.find(x => x.id === this.props.match.params.actID)
        ...
        if(associate !== undefined){
            ...
            indexGerant = this.state.gerants.indexOf(gerant);
            firebase.database().ref('/society/'+this.props.match.params.SocietyUID).child('sAdministrator').child(indexGerant).update({
                'signatureStatut':url
            }).then(res=> {
                console.log("ok")
                augmCapitalService.getEntrepriseStatut(this.state.uid, this.state.pays).then(res => {
                ...
                <div className="wrapper" style={{paddingTop: 100}}>
                    <Container fluid>
                        <Suspense fallback={loading()}>
                                        ...
                                        <div className="card-body" style={{padding: '0.9rem'}}>
                                            <div className="float-right">
                                                <button type="button"
                                                    ...
                                                    {
                                                        this.state.actionnaire.signatureStatut === "" ? "Signer le document" : "Vous avez signé le document"
                                                    }

                                                </button>
                                            </div>
                                            <h4 style={{fontWeight: 'bold'}} className="header-title mt-0 mb-3">Document
                                                de statut de la société {this.state.sName}</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <h4 className="header-title mt-0 mb-3" style={{fontWeight: "bold"}}>Signatures des gérants et des associés</h4>
                            <div style={{height: 1, backgroundColor: "#C0C0C0"}}/>
                            <div className="row" style={{marginTop: 20}}>
                                {
                                    this.state.allActionnaire.map((item, key) => (

                                        item.fullname= item.firstname + " " + item.lastname,
                                        item.signatureStatut !== "" ?
                                            ...
                                            <div className="col-md-4">
                                                ...
                                                <div className="card-box ribbon-box">
                                                    ...
                                                    <div className="text-center">
                                                        <img src={item.signatureStatut} alt="" height="65" width="200"
                                                             style={{
                                                                 objectFit: "none",
                                                                 marginTop: 5,
                                            ...
                                            item.signatureStatut === "" && item.id !== this.props.match.params.actID ?
...
export default signSARLDocStatut