...
class stockOptionSuissedetails extends Component {
    ...
    showDocument = (titrekey,opkey) => event => {
        ...
        let typeActio = "";
        if(this.props.location.state.typeActio === "associate"){
            typeActio = "associate"
        }else{
            typeActio = "admin"
        }
        augmCapitalService.getBSASuisseDoc(localStorage.getItem(("uid"))+"Suisse",this.props.location.state.indexActio,typeActio,titrekey,opkey).then( res => {
    ...
    render() {
        ...
        let actio = {};
        if (typeActio === "admin")
            actio = entreprise.sAdministrator[indexActio];
        else
            actio = entreprise.sAssociate[indexActio];
                                            ...
                                            this.state.showInformations && operations.map((item, key) => (
                                                ...
                                                {item.nb}
                                                                ...
                                                                <span className="badge bg-soft-info text-info p-1"
                                                                      style={{marginLeft: 8}}>Actions simples</span>
                                                            </div>
                                                        </div>
                                                        <div className="col-md-6">
