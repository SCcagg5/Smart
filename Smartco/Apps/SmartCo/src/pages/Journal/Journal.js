    ...
    render() {
        ...
        return(
            <Container>
                ...
                <Content>
                    <View>
                        <Divider style={{ backgroundColor: '#fff',height:25,marginTop:20}}>
                            <Text style={{marginLeft:15,fontSize:18,fontWeight:'bold',color:'#00cc7f'}}>
                                Sélectionner une opération</Text>
                        </Divider>
                        {
                            operationsList.map((l, key) => (
                                <ListItem
                                    //leftAvatar={{source: l.image, overlayContainerStyle: {backgroundColor: '#fff'}}}
                                    key={key}
                                    title={l.title}
                                    titleStyle={{fontWeight:"bold",fontSize:15}}
                                    rightElement={<Image style={{height:40,width:40}} source={require("../../assets/icons/chevronright.png")}/>}
                                    subtitle={l.desc}
                                    onPress={() => this.props.navigation.navigate("SelectSociety")}
                                />
                            ))
                        }
