...
Sport.create = function (newEmp, result) {

    console.log(newEmp)

    dbConn.query("INSERT INTO sport set ?", newEmp, function (err, res) {
        if(err) {
...
Sport.findById = function (id, result) {
    dbConn.query("Select * from sport where question_id = ? ", id, function (err, res) {
        if(err) {
            console.log("error: ", err);
            result(err, null);
        }
...
Sport.update = function(id, employee, result){
...
Sport.delete = function(id, result){
