...
let augmCapitalService = {
    ...
    getBSASuisseDoc(uid,titrekey,opkey){

        console.log(endpoint+'/creationBSASuisseGET/' + uid +'/' + titrekey + '/' + opkey);
        return fetch(endpoint+'/creationBSASuisseGET/' + uid +'/' + titrekey + '/' + opkey, {
            method: 'GET',
        }).then(response => response.blob()
        ).catch(error => {
            console.log(error);
