    ...
    const validate_augmCapital2 = () => {
        ...
        let generation_data = {
            "data":{
                ...
                total_prime_emission : newActionsEmis * parseFloat(primeEmission),
                new_actios_desc:new_actions_desc_profit,
                total_prime_augm:(newActionsEmis * parseFloat(primeEmission)) + (newActionsEmis * parseFloat(selected_soc.capital.valeurAction)),
                new_parts:new_parts,
                prime_emission:primeEmission,
        ...
        setTimeout(() => {
            DocGenerationService.generate_TN_AugmCapital_primeEmission(generation_data).then( async genRes => {
                if(genRes && genRes.data){
            ...
            <div style={{
                                    ...
                                    <tbody>
                                    {
                                        associes.map((item, key) => (
                                            ...
                                            <tr>
                                                ...
                                                <td align="center">
                                                    <img src={(item.nationality === "française") ?
                                                        france : (item.nationality === "Suisse") ? suisse : tunisie}
                ...
                <div className="row">
                    <div className="col-md-12" align="center">
                        <button type="button"
                                disabled={date_ass === null || date_ass === undefined || totalNewActionsEmis <= 0}
                                className="btn btn-danger waves-effect waves-light btn-lg"
                                onClick={() => {
                                    if(typeAugCapital === "1"){
                                        validate_augmCapital1()
                                    }else{
                                        if(primeEmission === "" || isNaN(primeEmission) || newAssociesValidated.length === 0){
                                            props.openSnackbar("error","La valeur de la prime d'emission est invalide ou Auncun nouveau associé encore ajouté !")
                                        }else{
